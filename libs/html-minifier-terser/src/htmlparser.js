...
class HTMLParser {
  ...
  parse() {
      ...
      if (!lastTag || !special(lastTag)) {
        ...
        if (textEnd === 0) {
          ...
          if (endTagMatch) {
            ...
            replaceAsync(endTagMatch[0], endTag, parseEndTag)
        ...
        html = replaceAsync(html, reStackedTag, (_, text) => {
    ...
    function handleStartTag(match) {
      ...
      const unary = empty(tagName) || tagName === 'html' && lastTag === 'head' || Boolean(unarySlash)

      const attrs = match.attrs.map((args) => {
        let customAssign; let customClose; let customOpen; let name; let quote; let
          value
