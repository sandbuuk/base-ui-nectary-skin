...
const endTag = require('./htmlparser').endTag
...
function cleanConditionalComment(comment, options) {
  return options.processConditionalComments ? utils.replaceAsync(comment, /^(\[if\s[^\]]+]>)([\s\S]*?)(<!\[endif])$/, (match, prefix, text, suffix) => {
  ...
  Object.keys(values).forEach((key) => {
    ...
    if (key === 'caseSensitive') {
      ...
      options.minifyCSS = function(text, type) {
        text = text.replace(/(url\s*\(\s*)("|'|)(.*?)\2(\s*\))/ig, (match, prefix, quote, url, suffix) => {
...
function minify(value, options, partialMarkup) {
  ...
  const customFragments = options.ignoreCustomFragments.map((re) => {
    return re.source
  })
  ...
  const parser = new HTMLParser(value, {
    ...
    chars(text, prevTag, nextTag) {
      ...
      if (uidPattern && options.collapseWhitespace && stackNoTrimWhitespace.length) {
        text = text.replace(uidPattern, (match, prefix, index) => {
          return ignoredCustomMarkupChunks[Number(index)][0]
        })
      }
