import { test } from '@playwright/test'
import { makeAccessibilityTests } from '../accessibility-tests'
import { makeScreenshotTests } from '../screenshot-tests'

const imageData = 'data:image/jpeg;base64,'

const withAlt = makeScreenshotTests('/avatar?alt=NS', 'sinch-avatar')
const check = makeAccessibilityTests('/avatar?alt=NS', 'sinch-avatar')
const sizeValues = ['l', 'm', 's'] as const
const backgroundValues = ['blue', 'yellow', 'grey'] as const

test('accessibility', check(async function* () {
  yield
}))

test('size property', withAlt(async function* ({ $eval }) {
  for (const val of sizeValues) {
    await $eval((el, val) => {
      el.size = val
    }, val)
    yield { name: val }
  }
}))

test('size attribute', withAlt(async function* ({ $eval }) {
  for (const val of sizeValues) {
    await $eval((el, val) => {
      el.setAttribute('size', val)
    }, val)
    yield { name: val }
  }
}))

test('background property', withAlt(async function* ({ $eval }) {
  for (const val of backgroundValues) {
    await $eval((el, val) => {
      el.background = val
    }, val)
    yield { name: val }
  }
}))

test('background attribute', withAlt(async function* ({ $eval }) {
  for (const val of backgroundValues) {
    await $eval((el, val) => {
      el.setAttribute('background', val)
    }, val)
    yield { name: val }
  }
}))

test('alt property', withAlt(async function* ({ $eval }) {
  await $eval((el) => {
    el.alt = 'XX'
  })
  yield { name: 'updated' }
}))

test('alt attribute', withAlt(async function* ({ $eval }) {
  await $eval((el) => {
    el.setAttribute('alt', 'XX')
  })
  yield { name: 'updated' }
}))

test('src attribute', withAlt(async function* ({ $eval }) {
  await $eval((el, imageData) => {
    el.setAttribute('src', imageData)
  }, imageData)
  yield { name: 'set' }
}))

test('src property', withAlt(async function* ({ $eval }) {
  await $eval((el, imageData) => {
    el.src = imageData
  }, imageData)
  yield { name: 'set' }
}))
